  ...
  Future<void> triggerOrderStatusNotification({
    ...
    try {
      ...
      await _client.from('notifications').insert({
        ...
        'body':
            bodyMap[newStatus] ?? 'Your order status changed to $newStatus.',
        'type': 'order_status',
        'is_read': false,
      });
      ...
      _log.severe(
        'Failed to create notification: orderId=$orderId, error=${e.message}',
        e,
      );
      throw ServerException(message: e.message, statusCode: e.code);
    } catch (e, st) {
      _log.severe(
          'Unexpected error creating notification: orderId=$orderId', e, st,);
